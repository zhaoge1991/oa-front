...
export class OrderService {
    ...
    //获取订单产品
    getProductList(page?: number, key?: string) {
        if (key) {
            return this.http.get('/api/procurement/order/products' + '?page=' + page + '&keyword=' + key).map(res => {
                return res.json();
            });
        } else {
            return this.http.get('/api/procurement/order/products' + '?page=' + page).map(res => {
                return res.json();
            });
        }
    }
     ...
     * 生成采购单
     */
    toProcurementOrder(order_products){
        return this.http.post('/api/procurement/order/to_procurement_order',{'order_products':order_products}).map(res => {
            return res.json();
        });
    }
    /**
     * 生成冻结单
     */
    toFreezeOrder(order_products){
        return this.http.post('/api/procurement/order/to_freeze_order',{'order_products':order_products}).map(res => {
            return res.json();
        });
    }
